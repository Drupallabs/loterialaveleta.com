...
class ImporterManager extends DefaultPluginManager
{
    ...
    {
        parent::__construct('Plugin/Importer', $namespaces, $module_handler, 'Drupal\sorteos\Plugin\ImporterInterface', 'Drupal\sorteos\Annotation\Importer');

        $this->alterInfo('sorteos_importer_info');
        $this->setCacheBackend($cache_backend, 'sorteos_importer_plugins');
     ...
     *
     * @return null|\Drupal\sorteos\Plugin\ImporterInterface
     */
    public function createInstanceFromConfig($id)
    {
        $config = $this->entityTypeManager->getStorage('importer')->load($id);
        if (!$config instanceof \Drupal\sorteos\Entity\ImporterInterface || !$config->getActive()) {
            return NULL;
        }
     ...
     *
     * @return \Drupal\sorteos\Plugin\ImporterInterface[]
     */
